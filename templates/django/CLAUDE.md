...
> **AI 에이전트 필독**: 작업 전 관련 docs/ 문서를 먼저 참조하라. 관련 문서가 없으면 architect가 작업 후 생성한다.
...
- **architect**: 아키텍처 결정·복잡한 비즈니스 정책이 코드에 반영될 때 해당 문서를 `docs/`에 생성 또는 갱신
- **reviewer**: PR에 docs/ 갱신이 누락됐는지 확인. 누락 시 보완 요청

**프로젝트 참고 문서** (아래에 주요 문서를 나열하고 관리한다):

| 카테고리 | 문서 | 설명 |
|---------|------|------|
| 아키텍처 | `docs/DOC-SYNC-POLICY.md` | 문서 동기화 정책 |
| (TODO) | — | 작업 진행하며 추가 |
