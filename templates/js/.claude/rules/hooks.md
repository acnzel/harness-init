...
| 이벤트 | 훅 | 동작 |
|--------|-----|------|
| PreToolUse(Bash) | `pre-bash-guard.sh` | DROP/WHERE없는DELETE 전 경고 출력 |
...
`domain-guard.sh` 는 **편집한 파일 하나만** 판정하고, 선언 블록 지문을 변경 전후로 비교한다.
주석·포맷·리팩토링에는 발화하지 않는다. 발화했다면 enum·union·`as const`·`@@map` 중
하나가 실제로 바뀐 것이니 DOMAIN.md를 갱신해야 한다. 자세한 내용은 `@.claude/rules/knowledge.md`.
...
출력하지 말아야 할 때:
- TypeScript, React, NestJS 등 일반 라이브러리 사용법
