...
| 변경된 코드 위치 | 갱신해야 할 문서 |
|----------------|----------------|
| `app/api/**/route.ts` | `docs/api/{feature}.md` |
| `app/**/actions.ts` | `docs/api/{feature}.md` (Server Action이 외부 계약을 바꾸면) |
| `lib/**/repository.ts`, `schema.prisma` | ADR (스키마 변경 시) |
| `lib/**/service.ts` | 비즈니스 로직 변경이 API 동작에 영향 시 `docs/api/` |
...
- **동작**:
  1. 변경된 파일 목록 분석
  2. `route.ts` / `actions.ts` 변경 감지 → API 문서 갱신 이슈 자동 생성
